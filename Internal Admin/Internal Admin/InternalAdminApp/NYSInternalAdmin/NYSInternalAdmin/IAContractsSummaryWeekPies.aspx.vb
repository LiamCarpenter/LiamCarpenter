...
Partial Public Class IAContractsSummaryWeekPies
    Inherits clsNYS
        ...
        Try
            If Not IsPostBack Then
                ...
                If blnDefault Then
                    ...
                    setLabels(dt.Month, dt.Year)
                End If
                lblBookingDetails.Text = "Contracts Summary Weekly Charts - " & ddreport.SelectedItem.Text
            End If
            Me.Title = CStr(Session.Item("clientname")) & " Management Information by NYS Corporate"
        Catch ex As Exception
            handleexception(ex, "IAContractsSummaryWeekPies", Me.Page)
        End Try
    End Sub

    Private Sub setLabels(ByVal pintMonth As Integer, ByVal pintYear As Integer)
        Using New clslogger(log, className, "setLabels")

            Dim startDate As Date = New Date(pintYear, pintMonth, 1)
            ...
            Else
                If startDate.DayOfWeek = DayOfWeek.Monday Then
                    ...
                    Friday = startDate
                Else
                    While startDate.DayOfWeek = DayOfWeek.Monday Or startDate.DayOfWeek = DayOfWeek.Friday
                        startDate = startDate.AddDays(1)
                    End While

                    If startDate.DayOfWeek = DayOfWeek.Friday Then
                        Monday = New Date(pintYear, pintMonth, 1)
                        Friday = startDate
                    Else
                        Monday = startDate
                        While startDate.DayOfWeek <> DayOfWeek.Friday
                ...
                End If
            End If
            lblweek1.Text = Monday.Day & " - " & Friday.Day & " " & getMonthAsString(Monday.Month)
            'now get the next monday then friday
            ...
            End While
            Friday = startDate
            lblweek2.Text = Monday.Day & " - " & Friday.Day & " " & getMonthAsString(Monday.Month)
            ...
            End While
            Friday = startDate
            lblweek3.Text = Monday.Day & " - " & Friday.Day & " " & getMonthAsString(Monday.Month)
            ...
            lblweek4.Text = Monday.Day & " - " & Friday.Day & " " & getMonthAsString(Monday.Month)
            ...
            If startDate < endDate Then
                ...
                Else
                    Friday = startDate
                End If
                lblweek5.Text = Monday.Day & " - " & Friday.Day & " " & getMonthAsString(Monday.Month)
            Else
                lblweek5.Text = ""
            End If
        End Using
    ...
    Protected Sub btnrefresh_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnrefresh.Click
        Using New clslogger(log, className, "btnrefresh_Click")
            Try
                ...
                lblBookingDetails.Text = "Contracts Summary Weekly Charts - " & ddreport.SelectedItem.Text
                rvWeek1.LocalReport.Refresh()
                rvWeek2.LocalReport.Refresh()
                rvWeek3.LocalReport.Refresh()
                rvWeek4.LocalReport.Refresh()
                rvWeek5.LocalReport.Refresh()
            Catch ex As Exception
                handleexception(ex, "IAContractsSummaryWeekPies", Me.Page)
            End Try
        End Using
    ...
    Private Sub IAContractsSummaryWeekPies_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRender
        Using New clslogger(log, className, "IAContractsSummaryWeekPies_PreRender")
            Try
                Dim fp As ucReportMenu = DirectCast(LoadControl("UserControls/ucReportMenu.ascx"),  _
                    ucReportMenu)
                fp.pageName = "IAContractsSummaryWeekPies"
