...
Partial Public Class IABlanketPoCheck
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Using New clslogger(log, className, "Page_Load")
            ...
            Catch ex As Exception
                handleexception(ex, "IABlanketPoCheck", Me.Page)
            End Try
        End Using
    End Sub
    ...
    Private Sub IABlanketPoCheck_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRender
        Using New clslogger(log, className, "IABlanketPoCheck_PreRender")
            Try
                Dim fp As ucReportMenu = DirectCast(LoadControl("UserControls/ucReportMenu.ascx"),  _
                                      ucReportMenu)
                fp.pageName = "IABlanketPoCheck"
    ...
    Private Sub checkBlanketPO(ByVal pstrRequesterName As String)
        Try
            Using New clslogger(log, className, "checkPO")

                Dim oPos As New List(Of clsPo)
                oPos = clsPo.poBlanketList(pstrRequesterName)
                ...
                oBPOs = clsPo.POBookedBlanketCheck
                ...
                oBPXs = clsPo.POImportedBlanketCheck
        ...
        Using New clslogger(log, className, "btnrefresh_Click")
            Try
                If ddpoOwners.SelectedIndex < 1 Then
                    checkBlanketPO("")
                Else
                    checkBlanketPO(ddpoOwners.SelectedItem.Text)
                End If
            Catch ex As Exception
                handleexception(ex, "IABlanketPoCheck", Me.Page)
            End Try
